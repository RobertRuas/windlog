...
- Frontend pages follow `{PageName}.tsx + components/ + hooks/`; services live in `src/services/` and encapsulate API calls via the shared `api.ts` client. Large pages (>500 lines) must extract sub-components, types, and helpers into dedicated files.
...
- All user-facing text MUST use i18n `t()` function — no hardcoded strings in JSX. Exception: timesheet export content is intentionally English-only (international document).
- Frontend comments and documentation headers must be in Portuguese (didactic style explaining "O QUE É ESTE ARQUIVO?").
- Utility files follow dot-notation naming: `name.helpers.ts`, `name.types.ts`.